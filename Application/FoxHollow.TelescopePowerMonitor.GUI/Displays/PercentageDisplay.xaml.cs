...
namespace FoxHollow.TelescopePowerMonitor.GUI
{
    ...
    public partial class PercentageDisplay : UserControl
    {
        ...
        private void ValueChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                this.Current.Text = Formatting.FormatPercentage(this.Value.Current);
                this.Minimum.Text = Formatting.FormatPercentage(this.Value.Minimum);
                this.Maximum.Text = Formatting.FormatPercentage(this.Value.Maximum);
        ...
        private static void OnValuePropertyChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e)
        {
            if (e.Property.Name == nameof(Value))
                (e.NewValue as FloatValue).PropertyChanged += ((PercentageDisplay)sender).ValueChanged;

            if (e.Property.Name == nameof(IsActive) && ((bool)e.NewValue) == false)
                ((PercentageDisplay)sender).ResetView();
        }

        public static readonly DependencyProperty ValueProperty = DependencyProperty.Register("Value", typeof(FloatValue), typeof(PercentageDisplay), new UIPropertyMetadata(new FloatValue(), OnValuePropertyChanged));
        public static readonly DependencyProperty IsActiveProperty = DependencyProperty.Register("IsActive", typeof(bool), typeof(PercentageDisplay), new UIPropertyMetadata(false, OnValuePropertyChanged));
        public static readonly DependencyProperty TitleProperty = DependencyProperty.Register("Title", typeof(string), typeof(PercentageDisplay), new UIPropertyMetadata(" ", OnValuePropertyChanged));
    }
}
